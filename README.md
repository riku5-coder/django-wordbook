...
## 工夫した点
- core\utils.pyに入っているlookup_enja関数で単語の意味を検索します。ExcelAPIに単語の意味をリクエストすると（json形式などではなく）ただの文字列データが返ってくるので、それをリスト化して使いやすいようにしています
- 単語の意味の検索と登録がダイレクトにつながっており、複数意味がある場合はその一つだけを選んだりすべてを選んだり編集したりして登録できます
  - htmlテンプレート上でforloop.last、forloop.counterなどを使って意味の表示の仕方を工夫しました
...
- 単語のリストやフラッシュカードでは、登録されたとき（「今日」「直近一週間」「直近一か月」）によって単語をソートできるようになっています
...
## その他
 ...
 - 通常cofigと名付けられやすいフォルダはここではwordwordという名になっています。
